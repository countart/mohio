...
**Compiled 2026-08-11**, quoted from `mohio_data/mohio.lark` as it stood that day. Usage was
measured across all 35 real `.mho` files that shipped with the language at the time, leaving out
drafts and throwaway scratch files. This file is the raw, verified data underneath that count:
which verbs have an inline form, which have a block form, and which have both.
