...
>> **Language Reference** (`tools/langref/out/LANGUAGE-REFERENCE.md`), which is produced
>> from the grammar and is always current. Use that for canonical syntax. This file is
>> kept for historical context only. **It contains retired forms, aspirational features that
>> never shipped, and claims that are false against the current compiler.** For example, the
>> "Built-In Shapes" table below is wrong — there are **no** magic built-in shapes (`sh.Member`
>> is not available unless you declare `shape Member`; you declare every shape yourself), and
...
>> "prefix system" language throughout this file are RETIRED, INCORRECT framing.
>> The dot is a GROUPING
...
*A quick reference for the Mohio language surface. Where this document and the generated
language reference (`start-here/LANGUAGE-REFERENCE.md`) disagree, the generated reference is
right: it is built from the grammar itself. An older Language Design Document (LDD) is
outdated at every version and is not a source for anything here.*
...
MioQL feeds the arc. AI works on what MioQL returned. Always sequential — never nested. A `find` block never contains `ai.decide`. An `ai.decide` block never contains `find`.
...
| `create` | Build an object from pieces / known shapes (was `make`, retired). `ai.create` builds from nothing via AI. |
...
> `Docs/guides/mioql-user-guide.md`). `match.unique` itself carries no special meaning there since
...
> `Docs/guides/Mohio-Language-Reference-Pioneer-Edition.md`), not the inline `match.any` modifier.
> `match.none`/`match.all` as inline modifiers have no working equivalent today. Zero real
> usage of the inline modifier forms was found in any real program either.
...
*The generated language reference (`start-here/LANGUAGE-REFERENCE.md`) is built from the
grammar. When this document conflicts with it, this document is wrong.*
