...
brief. The reason the dot exists at all is that it is a **grouping directive**, not "a modifier
attached to a verb": it joins two or more words into one unambiguous unit. A space leaves the
grouping open, so under Earley the parser must explore every possible grouping, and the wrong
one can silently win. Two-word spellings work ONLY where the grouping is forced some other way,
regardless of the dot. Everywhere grouping is genuinely ambiguous, two-word either hard-fails or
silently misparses, which was measured by running every form rather than reasoning about it.
...
`retrieve all`, `modify as`, `apply in`, and the rest of the 272 measured sites either
hard-fail or silently misparse today. Write the dot form.
